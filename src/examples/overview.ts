...
import {
    ...
    imStr,
    newDomAppender
} from "src/utils/im-js";
import {
    imBaseContainerBegin,
    imBaseContainerEnd,
    imHeadingBegin,
    imHeadingEnd,
    imParaBegin,
...
export function imJsCompleteOverview(c: ImCache, harness: VisualTestHarnessState) {
    imBaseContainerBegin(c); {
        ...
        imParaBegin(c); {
            imStr(c, `imJS is actually written entirely in TypeScript. I just didn't like the name imTS very much. In order to use it, you'll need to copy the im-js folder into somewhere in your project. im-core.ts, im-dom.ts and assert.ts are really the only files you actually need, the rest are optional. `);
        ...
        imVisualTestInstallation(c, "use imSwitch instead", harness, imSwitchExample, TEST_CENTERED);
        ...
        imParaBegin(c); {
            imStr(c, `Hey what gives! Disabling mouse tracking didn't remove the event?! Right now, if-statements will only detatch their DOM nodes, but keep state around for performance reasons. Destructors only run when an item gets destroyed. Right now, keyed list items, as well as and anything rendered under imSwitch, will be _destroyed_ by default to avoid memory leaks. I figured this was a decent compromise. This does mean that destructors should solely be used to free up resources/clean up memory leaks, and not to reliably run application logic when a component is detatched/destroyed. For now, if you really need to rely on the destructor to run when a component detatches, you'll have to write it to use imSwitch (sorry):`);
        } imParaEnd(c);
        ...
        imVisualTestInstallation(c, "imMemo 90% of usecases", harness, imMemoExamples, TEST_CENTERED);
